...
约束：
1. 如果用户只是打招呼、闲聊、感谢、测试在线状态，steps 只能包含 intent.analyze、planner.build、sync.broadcast，不能包含 doc.create、doc.append、slide.generate、archive.bundle；deliverables 必须为空数组，documentSections 和 slides 必须为空数组。
...
只输出严格 JSON，不要输出 Markdown，不要包裹代码块。JSON 结构如下：
{
  ...
  "steps": [
    ...
    {"id":"r2","tool":"doc.update","description":"更新已有文档","args":{"revision":"用户更新要求"},"dependsOn":["r1"]},
    {"id":"r3","tool":"slide.regenerate","description":"更新已有 PPTX","args":{"revision":"用户更新要求"},"dependsOn":["r2"]}
  ],
  "docTitle": "沿用或调整后的文档标题",
  "slideTitle": "沿用或调整后的演示稿标题",
  "documentSections": [
    {"heading":"Revision request","bullets":["用户更新要求"]}
  ],
  "slides": [
    {"title":"更新说明","bullets":["用户更新要求"]}
  ]
}
...
4. 不要虚构具体事实；缺失信息放到 clarifyingHint 或 risks。`
