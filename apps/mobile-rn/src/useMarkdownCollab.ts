...
export function useMarkdownCollab(taskId: string | undefined, clientId: string) {
  ...
  const bootRetryTimerRef = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  const catchupTimerRef = useRef<ReturnType<typeof setInterval> | undefined>(undefined);
    ...
    async function boot() {
      try {
        ...
        if (!closed) {
          ...
          bootRetryTimerRef.current = setTimeout(() => void boot(), 2000);
        }
      }
    }
